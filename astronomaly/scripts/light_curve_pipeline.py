...
def run_pipeline():
    ...
    if len(input_dir) != 0:
        light_curve_dataset = light_curve_reader.LightCurveDataset(directory=os.path.join(input_dir,'light_curves'))

        features = pd.read_csv(os.path.join(input_dir,'feature_table_large_mag_zeros_remove.ascii'), delim_whitespace=True)
        ...
        return {'dataset':light_curve_dataset, 'features':features, 'anomaly_scores': anomalies,
