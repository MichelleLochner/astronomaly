...
lc_path = os.path.join(data_dir, 'plasticc', 'subset_500.csv')

# Where output should be stored
output_dir = os.path.join(
    data_dir, 'astronomaly_output', 'plasticc', '')
...
def run_pipeline():
    ...
    lc_dataset = light_curve_reader.LightCurveDataset(
        ...
        plot_errors=False,
        plot_column='flux')
    # print(lc_dataset.get_sample(lc_dataset.index[0]))
    ...
    # features = pd.DataFrame(data={'0':np.random.randn(len(lc_dataset.index)),
    #                               '1':np.random.randn(len(lc_dataset.index))},
    #                         index = lc_dataset.index)
    # print('features')
    # print(features)
