...
from astronomaly.anomaly_detection import isolation_forest, human_loop_learning, lof
from astronomaly.clustering import tsne

input_file = '/home/michelle/Project/OutlierDetection/outlier_detection/data/y_test.npy'
...
def run_pipeline():
    ...
    raw_dataset = raw_features.RawFeatures(filename=input_file, output_dir=output_dir)
    ...
    pipeline_active_learning = human_loop_learning.NeighbourScore(alpha=1, force_rerun=True, output_dir=output_dir)

    pipeline_tsne = tsne.TSNE_Plot(output_dir=output_dir, perplexity=50)
    t_plot = pipeline_tsne.run(features)

    return {'dataset': raw_dataset, 'features': features, 'anomaly_scores': anomalies,
